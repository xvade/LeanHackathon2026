...
Key features compared to `neural_grind` / `NeuralTactic.Tactic`:
  ...
  • When `$GRIND_MODEL` and `$GRIND_SERVE` are both set, the tactic spawns
    the configured Python server as a persistent subprocess and queries it for model scores.
...
Environment variables:
  ...
  GRIND_SERVE — path to the Python model server (required when GRIND_MODEL is set)
