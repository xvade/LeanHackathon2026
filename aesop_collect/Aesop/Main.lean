...
meta def evalAesopCore (collectUnsafeTrace : Bool) : Tactic := λ stx => do
      ...
      profiling (λ s _ t => { s with search := t }) do
        let (goals, stats) ←
          search goal ruleSet config.options config.simpConfig
            ...
            (collectUnsafeTrace := collectUnsafeTrace)
...
meta def evalAesop : Tactic :=
  evalAesopCore (collectUnsafeTrace := false)

@[tactic Frontend.Parser.aesopCollectTactic]
meta def evalAesopCollect : Tactic :=
  evalAesopCore (collectUnsafeTrace := true)
