...
structure RappData (Goal MVarCluster : Type) : Type where
  ...
  /--
  If this rapp was produced by an unsafe-rule choice, this is the complete
  unsafe queue visible at the moment the rule was selected. The chosen rule is
  `appliedRule`.
  -/
  unsafeCandidates? : Option (Array RuleName)
...
@[inline]
def unsafeCandidates? (r : Rapp) : Option (Array RuleName) :=
  r.elim.unsafeCandidates?
...
@[inline]
def setUnsafeCandidates? (unsafeCandidates? : Option (Array RuleName))
    (r : Rapp) : Rapp :=
  r.modify λ r => { r with unsafeCandidates? }
