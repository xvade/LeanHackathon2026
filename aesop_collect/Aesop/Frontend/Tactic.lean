...
/--
`aesop_collect <clause>*` behaves like `aesop`. If the search closes the goal,
it additionally logs the unsafe-rule choices on the final proof branch. Each
entry contains the goal at the choice point, the unsafe rules available at that
point, and the unsafe rule that was selected.
-/
syntax (name := aesopCollectTactic)
  "aesop_collect" (ppSpace colGt Aesop.tactic_clause)* : tactic

meta initialize do
  Batteries.Linter.UnreachableTactic.addIgnoreTacticKind ``aesopTactic
  Batteries.Linter.UnreachableTactic.addIgnoreTacticKind ``aesopTactic?
  Batteries.Linter.UnreachableTactic.addIgnoreTacticKind ``aesopCollectTactic
...
def parse (stx : Syntax) (goal : MVarId) : TermElabM TacticConfig :=
  withRef stx do
    ...
    | `(tactic| aesop_collect $clauses:Aesop.tactic_clause*) =>
      go (traceScript := false) clauses
