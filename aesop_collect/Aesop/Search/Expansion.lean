...
def addRapps (parentRef : GoalRef) (rule : RegularRule)
    ...
    (unsafeCandidates? : Option (Array RuleName) := none) :
...
def runUnsafeRule (parentRef : GoalRef)
    ...
    (unsafeCandidates : Array RuleName) : SearchM Q RuleResult := do
  let rule := matchResult.rule
  profilingRule (.ruleName rule.name) (·.isSuccessful) do
    withRuleTraceNode rule.name (·.toEmoji) "" do
      ...
      addRapps parentRef (.unsafe rule) output.applications
        (unsafeCandidates? := some unsafeCandidates)
...
partial def runFirstUnsafeRule (postponedSafeRules : Array PostponedSafeRule)
  ...
  let unsafeCandidates := queue.toArray.map (·.name)
  let (remainingQueue, result) ← loop unsafeCandidates queue
  ...
  return result
  where
    loop (unsafeCandidates : Array RuleName) (queue : UnsafeQueue) :
        SearchM Q (UnsafeQueue × RuleResult) := do
      let (some (r, queue)) := Subarray.popHead? queue
        | return (queue, RuleResult.failed)
      match r with
      | .unsafeRule r =>
        let result ← runUnsafeRule parentRef r unsafeCandidates
        match result with
        | .proved .. => return (queue, result)
        | .succeeded .. => return (queue, result)
        | .failed => loop unsafeCandidates queue
      | .postponedSafeRule r =>
        return (queue, ← applyPostponedSafeRule r parentRef unsafeCandidates)
