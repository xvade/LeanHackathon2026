...
private def formatRuleNames (rules : Array RuleName) : MessageData :=
  m!"[" ++ MessageData.joinSep (rules.toList.map (fun r => m!"{r}")) ", " ++ m!"]"

partial def collectUnsafeProofTraceFromGoal (gref : GoalRef) :
    SearchM Q (Array MessageData) := do
  let g ← gref.get
  let some rref ← g.firstProvenRapp?
    | return #[]
  let r ← rref.get
  let mut entries := #[]
  if r.appliedRule.isUnsafe then
    if let some candidates := r.unsafeCandidates? then
      let (goal, metaState) ← g.currentGoalAndMetaState (← getRootMetaState)
      let goalMessage ← metaState.runMetaM' do
        goal.withContext do
          addMessageContext (toMessageData goal)
      entries := entries.push $
        m!"(goal :={indentD goalMessage},\n  allowedUnsafeRules := {formatRuleNames candidates},\n  chosen := {r.appliedRule.name})"
  for cref in r.children do
    let childGoalRef? ← (← cref.get).provenGoal?
    if let some childGoalRef := childGoalRef? then
      entries := entries ++ (← collectUnsafeProofTraceFromGoal childGoalRef)
  return entries

def traceUnsafeProofChoices : SearchM Q Unit := do
  let entries ← collectUnsafeProofTraceFromGoal (← getRootGoal)
  let msg :=
    if entries.isEmpty then
      m!"aesop_collect: []"
    else
      m!"aesop_collect: [" ++
        indentD (MessageData.joinSep entries.toList ",\n") ++
        m!"\n]"
  logInfo msg
...
def finishIfProven : SearchM Q Bool := do
  unless (← (← getRootMVarCluster).get).state.isProven do
    return false
  finalizeProof
  if (← read).collectUnsafeTrace then
    traceUnsafeProofChoices
...
def search (goal : MVarId) (ruleSet? : Option LocalRuleSet := none)
  ...
  let go : SearchM Q _ := do
    try
      ...
      freeTree
  let ((goals, _, _), stats) ←
    go.run ruleSet options simpConfig simpConfigSyntax? collectUnsafeTrace goal |>.run stats
  return (goals, stats)
