...
# Plus 5 new heuristics: tryPostpone (1), variant_onehot (3), isGrindChoice (1)
NUMERIC_DIM = 17 + 7 + 3 + 5  # 32

VARIANT_TAGS = ["default", "imp", "arg"]

def new_heuristics(c: dict) -> list[float]:
    variant = c.get("variant", "")
    var_vec = [1.0 if variant == t else 0.0 for t in VARIANT_TAGS]
    return [
        1.0 if c.get("tryPostpone", False) else 0.0,
        *var_vec,
        1.0 if c.get("isGrindChoice", False) else 0.0
    ]
...
def batch_numeric(cands: list[dict], goal: dict,
    ...
    heur   = [new_heuristics(c) for c in cands]
    combined = [b + a + counts + h for b, a, h in zip(base, agg, heur)]
