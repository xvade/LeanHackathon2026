...
def train(args):
    ...
    groups: dict[int, list[tuple[torch.Tensor, int]]] = defaultdict(list)
    for ex in examples:
        ...
        groups[int(numeric.shape[0])].append((numeric, int(ex["target"])))
    print(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total = 0
        model.train()
        batches: list[list[tuple[torch.Tensor, int]]] = []
        ...
        for batch in batches:
            ...
            numeric = torch.stack([x for x, _ in batch], dim=0).to(device)
        ...
        print(f"Epoch {epoch:3d}/{args.epochs}  "
              f"loss={total_loss/total:.4f}  acc={acc:.1f}%", flush=True)
