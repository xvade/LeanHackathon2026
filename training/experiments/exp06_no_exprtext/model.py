"""EXP-06: Ablate exprText — use zeros instead of candidate text embedding."""
import torch
...
class SplitRanker(nn.Module):
    def __init__(
        ...
        # exprText slot replaced by zeros, so combined_dim same as base
    ...
    def forward(self, numeric, text_ids, state_ids=None, grind_ids=None):
        N = numeric.size(0)
        # exprText contribution replaced with zeros (same dimension)
        cand_emb  = torch.zeros(N, self.emb.embedding_dim, device=self.fc1.weight.device)
