"""EXP-05: Ablate statePP — always pass zeros for proof-state context."""
import torch
...
class SplitRanker(nn.Module):
    ...
    def forward(self, numeric, text_ids, state_ids=None, grind_ids=None):
        ...
        if grind_ids is not None and grind_ids.numel() > 0:
            emb = self.emb(grind_ids)
            grind_ctx = torch.relu(self.grind_proj(emb.mean(dim=0, keepdim=True))).expand(N, -1)
        else:
