...
example {a b x : α} : swap a b x ≠ x ↔ a ≠ b ∧ (x = a ∨ x = b) := by grind
...
example (a b : α) (s : Finset α) : insert a (insert b s) = insert b (insert a s) := by grind
...
example {z : ℤ} : Even z.sign ↔ z = 0 := by grind
...
example (f : α → β) (s t : Set α) : f '' (s ∪ t) = f '' s ∪ f '' t := by grind
...
example [DecidableEq α] : s.biUnion (singleton : α → Finset α) = s := by grind
...
example {s : Set α} {f : α →₀ M} :
    ↑f.support ⊆ s ↔ ∀ a ∉ s, f a = 0 := by grind
...
example : Iic b \ Ioc a b = Iic (a ⊓ b) := by grind
...
example (h : min c d ≤ b) : Iic b ∪ Icc c d = Iic (max b d) := by grind
...
example : Xor' (¬a) (¬b) ↔ Xor' a b := by grind
...
example : Odd (m + n) ↔ (Odd n ↔ Even m) := by grind
...
example (h₁ : min a b ≤ max c d) (h₂ : min c d ≤ max a b) :
    Ico a b ∪ Ico c d = Ico (min a c) (max b d) := by grind
...
example : min a c < min b c ↔ a < b ∧ a < c := by grind
...
example : Odd n.natAbs ↔ Odd n := by grind
...
example (a b c : α) : Ico a b \ Ico a c = Ico (max a c) b := by grind
...
example : Ioo a₁ b₁ ∩ Ioo a₂ b₂ = Ioo (a₁ ⊔ a₂) (b₁ ⊓ b₂) := by grind
...
example (γ₁ : Path a₁ a₂) (δ₁ : Path a₂ a₃) (γ₂ : Path b₁ b₂)
    (δ₂ : Path b₂ b₃) : (γ₁.prod γ₂).trans (δ₁.prod δ₂) = (γ₁.trans δ₁).prod (γ₂.trans δ₂) := by grind
...
example : s ×ˢ t \ s₁ ×ˢ t₁ = s ×ˢ (t \ t₁) ∪ (s \ s₁) ×ˢ t := by grind
...
example {s : Finset α} {a : α} : s ⊆ {a} ↔ s = ∅ ∨ s = {a} := by grind
...
example {a : α} {s : Set α} (h : a ∈ s) : insert a s = s := by grind
...
example : (Ioc a b)ᶜ = Iic a ∪ Ioi b := by grind
...
example (a b c : α) : {x ∈ Ioo a b | x < c} = Ioo a (min b c) := by grind
...
example : Ioc a b ∪ Ioc b a = Ioc (min a b) (max a b) := by grind
...
example : Even (m - n) ↔ (Even m ↔ Even n) := by grind
...
example (x : ℝ) (hx : x + 1/x = 21) : x^4 + 1/(x^4) = 192719   := by grind
...
example (c d : ℝ) : (c + d) * Real.sqrt (c^4 - d^4) / (c^4 - d^4) = Real.sqrt (c^4 - d^4) / ((c^2 + d^2) * (c - d))   := by grind
...
example (r₀ r₁ α : ℝ)
  ...
  r₁ / r₀ = (1 - Real.sin α) / (1 + Real.sin α)   := by grind
...
example : ∀ A : Set (ℕ → ℝ), A = {x | ∀ n : ℕ, 0 ≤ x n} ↔ ∀ x : ℕ → ℝ, x ∈ A ↔ ∀ n : ℕ, 0 ≤ x n   := by grind
...
example (x : ℝ) : x^2 + 5*x + 6 = 0 ↔ x = -3 ∨ x = -2   := by grind
...
example (a b : ℝ) (h : |a| ≤ b) : -b ≤ a ∧ a ≤ b   := by grind
...
example (p q r a b : ℝ)
  (h₀ : p + q + r = 1 / a)
  (h₁ : p * q + q * r + r * p = b / a) :
  a * b = (p * q + q * r + r * p) / (p + q + r)^2   := by grind
...
example (x y z : ℝ)
  ...
  (h₂ : y^2 + z^2 = x^2)
  (h₃ : z^2 + x^2 = y^2) :
  (x^2 + y^2 + z^2) / x + (x^2 + y^2 + z^2) / y + (x^2 + y^2 + z^2) / z ≥ 2 * (x + y + z)   := by grind
...
example (x : ℝ) (hx : 0 < x) (h : 3 * x / Real.sqrt (3 * x + 10) = Real.sqrt (3 * x + 1) - 1) : x = 0 ∨ x = 5   := by grind
...
example (x y: ℝ) : (x + 1) * (y + 1) ≥ 4 * Real.sqrt (x * y) ↔ x * y + x + y + 1 ≥ 4 * Real.sqrt (x * y)   := by grind
...
example (x y : ℝ) (hx : abs (x + y) + abs (x - y + 1) = 2) : 5 / 6 ≤ abs (2 * x - y) + abs (3 * y - x) ∧ abs (2 * x - y) + abs (3 * y - x) ≤ 21 / 2   := by grind
...
example (a b : ℝ) (ha : a ≠ 0) : a^2 / (a^2 + a * b + b^2) = 1 / (1 + b / a + b^2 / a^2)   := by grind
...
example (x : ℝ) : x^3 - 6 * x^2 + 9 * x - 4 = 0 ↔ x = 1 ∨ x = 1 ∨ x = 4   := by grind
...
example (x y : ℝ)
  (h₀ : 0 ≤ x)
  (h₁ : 0 ≤ y) :
  Real.sqrt x ^ 2 = x ∧ Real.sqrt y ^ 2 = y   := by grind
...
example (a : ℝ) (x y : ℝ) (ha : a > 0) (hx : |x - 1| < a / 3) (hy : |y - 2| < a / 3) : |2 * x + y - 4| < a   := by grind
...
example (n : ℕ) (b : ℕ → ℕ) (h₁ : b 0 = 5) (h₂ : ∀ n, b (n + 1) - b n = (n + 6).choose 4) : b (n + 1) = b n + (n + 6).choose 4   := by grind
...
example (x y : ℝ) : |y| - |x| ≤ |x - y|   := by grind
...
example (P Q : Set α) (hPQ : P ⊆ Q) : P ∩ Q = P   := by grind
...
example (x : ℝ)
  ...
  ((x - 1) * (x + 1)) / ((x - 1) * (x^2 + x + 1)) + (Real.cos (x - 1)) / (3 * x^2) =
    (x + 1) / (x^2 + x + 1) + (Real.cos (x - 1)) / (3 * x^2)   := by grind
...
example (α β γ : ℝ) (h₁ : α * β + β * γ + γ * α = 0) (h₂ : α * β * γ = 1) : 1 / γ = 1 / -α + 1 / -β   := by grind
...
example (t : ℝ) : t * (t - 1) * (t + 2) * (19 * t - 30) = 0 ↔ t = 0 ∨ t = 1 ∨ t = -2 ∨ t = 30 / 19   := by grind
...
example (M x: ℝ) (g : ℝ → ℝ) (h₁ : |g x - M| < |M| / 2) : |g x| > |M| / 2   := by grind
...
example (a b c : ℝ) (h₁ : a * (1/b) = 1) (h₂ : b * (1/c) = 1) : c * (1/a) = 1   := by grind
...
example (x : ℝ) (hx : x ≠ 1) : (x^2 - 2) / (x - 1)^3 = -1 / (x - 1)^3 + 2 / (x - 1)^2 + 1 / (x - 1)   := by grind
...
example (x : ℝ) : abs x + x ^ 2 + abs (abs x - 1) + 6 * abs (x - 2) + abs (x ^ 2 - 1) + 3 * abs (2 * x + 1) ≥ 17   := by grind
...
example {x y z : ℝ} (h₀ : (x + 1) * y * z = 12) (h₁ : (y + 1) * z * x = 4)
    (h₂ : (z + 1) * x * y = 4) :
    (x = 2 ∧ y = -2 ∧ z = -2) ∨ (x = 1 / 3 ∧ y = 3 ∧ z = 3) := by grind
...
example (x : ℝ) (hx : x^2 + 3 * x ≠ 0 ∧ x^2 + 9 * x + 18 ≠ 0 ∧ x^2 + 15 * x + 54 ≠ 0) :
    1 / (x^2 + 3 * x) + 1 / (x^2 + 9 * x + 18) + 1 / (x^2 + 15 * x + 54) = 3 / (2 * x + 18) ↔ x = 2 := by grind
...
# -/
example (x y z : ℝ) :
    (|x| < |y - z| ∧ |y| < |z - x| ∧ |z| < |x - y|) ↔ False := by grind
...
C: $2$
D: $\frac{5}{2}$ -/
example {x y z : ℝ} (h1 : 1 / x + 1 / (y + z) = 1 / 2) (h2 : 1 / y + 1 / (z + x) = 1 / 3) (h3 : 1 / z + 1 / (x + y) = 1 / 4) : 2 / x + 3 / y + 4 / z = 2 := by grind
...
example {x y : ℝ} (h₀ : x + y = 4) (h₁ : x * y = -2) :
    x + x^3 / y^2 + y^3 / x^2 + y = 440 := by grind
...
example {x1 x2 k : ℝ} (hx1 : x1^2 - 8 * x1 + k = 0) (hx2 : x2^2 - 8 * x2 + k = 0)
    (hroots : x1 ≠ x2) (hsum : 1 / x1 + 1 / x2 = 2 / 3) : k = 12 := by grind
...
example {x y z : ℝ}
    ...
    (x = -1 ∧ y = -3 ∧ z = -5) ∨ (x = 1 ∧ y = 3 ∧ z = 5) := by grind
...
example (x y t : ℝ) :
    ¬(abs x < abs (y - t) ∧ abs y < abs (t - x) ∧ abs t < abs (x - y)) := by grind
...
example (a b c : ℕ) (h₀ : a + b + c = 60) (h₁ : a ≤ b + c)
    ...
    (h₆ : |(c : ℤ) - a| ≥ 3) :
    min (min a b) c ∈ Finset.Icc 3 17 := by grind
...
example (n : ℕ) : 2 ∣ n ↔ Even (n % 10) := by grind
...
example (a d : ℚ) (h₀ : a > 0) (h₁ : d > 0)
    ...
    (h₄ : 1 / (a + 2 * d) = 1 / 6 + d) :
    ∑ i ∈ Finset.range 4, (1 / (a + i * d)) = 25 := by grind
...
$$ -/
example (x y z : ℝ) :
    abs x + abs y + abs z ≤ abs (x + y - z) + abs (x - y + z) + abs (-x + y + z) := by grind
...
example (a : ℕ → ℕ → ℕ)
    ...
    ¬∃ i j, a i j = 1 ∧ a (i + 1) j = 1 ∧ a i (j + 1) = 1 ∧ a (i + 1) (j + 1) = 1 := by grind
...
example (x : ℝ) (hx : 6*x-5≠0 ∧ 5*x+4≠0 ∧ 5*x-4≠0 ∧ 4*x+5≠0) :
    1/(6*x-5) - 1/(5*x+4) = 1/(5*x-4) - 1/(4*x+5) ↔ x=9 ∨ x=0 ∨ x=1 := by grind
...
holds. -/
example (x : ℝ) :
    abs (abs (abs (2 - x) - x) - 8) ≤ 2008 ↔ x ≥ -1007 := by grind
