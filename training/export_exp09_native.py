"""Export an exp09 PyTorch checkpoint to the native C++ binary format."""
...
def export(model: Path, out: Path) -> None:
    ...
    if input_dim != 32:
        raise SystemExit(f"expected exp09 input dim 32, got {input_dim}")

    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open("wb") as f:
        f.write(struct.pack("<8sIII", b"NGEXP09\0", 1, input_dim, hidden_dim))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model", default="training/experiments/exp09_heuristics/model_overfit.pt")
    parser.add_argument("--out", default="training/experiments/exp09_heuristics/model_overfit.native.bin")
